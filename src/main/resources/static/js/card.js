	...
	methods: {
					...
					success: function(res) {
						if (res.code == 200) {
							...
							if (clickid){
								...
								let dyCrossUrl = ""
								if (dyCrossState) {
									dyCrossUrl = ctxPath + "common/helpSendUrl";
								} else {
									dyCrossUrl = "http://ufi.106rcs.cn/iot-wechat-douyin/common/helpSendUrl";
								}
								$.ajax({
									type: 'post',
									url: dyCrossUrl,
		...
		submit3() {
			...
			_this.vldQuality(function () {
				...
				$.ajax({
					type: 'post',
					url: ctxPath + 'electronCard/broadband',
					contentType: 'application/json',
					dataType: 'json',
					data: JSON.stringify(params),
					success: function(res) {
		...
		showAgreement5() {
			this.alert(agreementTxt5, "我知道了");
		},
		showAgreement6() {
			this.alert(agreementTxt6, "我知道了");
		},
		showAgreement7() {
			this.alert(agreementTxt7, "我知道了");
		},
		showAgreement8() {
			this.alert(agreementTxt8, "我知道了");
		}
	}
});
